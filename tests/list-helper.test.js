...
describe('blog with the most likes', () => {
    ...
    test('list has only one blog', () => {
        ...
        expect(result).toEqual(format(listWithOneBlog[0]))
    })

    test('many blogs', () => {
        const result = listHelper.favoriteBlog(blogsList)
        expect(result).toEqual(format(blogsList[2]))
    })

    test('many blogs with two equally most liked blog', () => {
        const result = listHelper.favoriteBlog(blogsListEqual)
        expect(result).toEqual(format(blogsList[2]))
    })
})